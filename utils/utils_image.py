...
def imsave_png_without_compression(img, img_path):
    img = np.squeeze(img)
    if img.ndim == 3:
        img = img[:, :, [2, 1, 0]]
...
# convert torch tensor to uint
def tensor2float(img, data_range):
    img = img.data.squeeze().float().clamp_(0, 1*data_range).cpu().numpy()
    if img.ndim == 3:
        img = np.transpose(img, (1, 2, 0))
    return img
...
'''
Reverses functionality of augment_img function
'''
def reverse_augment_img(img, mode=0):
    if mode == 0:
        return img
    elif mode == 1:
        return np.rot90(np.flipud(img), k=3)
    elif mode == 2:
        return np.flipud(img)
    elif mode == 3:
        return np.rot90(img)
    elif mode == 4:
        return np.rot90(np.flipud(img), k=2)
    elif mode == 5:
        return np.rot90(img, k=3)
    elif mode == 6:
        return np.rot90(img, k=2)
