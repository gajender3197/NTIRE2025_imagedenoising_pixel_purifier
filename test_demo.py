...
from utils.utils_image import augment_img, reverse_augment_img
...
def select_model(args, device):
    ...
    if model_id == 1:
        # Restormer
        from models.team01_pixel_purifiers import Restormer
        print("Model is running......")
        name, data_range = f"{model_id:02}_01_pixel_purifiers", 1.0
        model_path = os.path.join('model_zoo', 'team01_pixel_purifiers.pth')
        model = Restormer()
...
def select_dataset(data_dir, mode):
    if mode == "test":
        ...
        lsdir_paths = [
            (
                os.path.join(data_dir, f"LSDIR_DIV2K_Test_Sigma50/{i:07}.png"),
                os.path.join(data_dir, f"LSDIR_DIV2K_Test_Sigma50/{i:07}.png")
            ) for i in range(1, 101)
        ]

        for lsdir_path in lsdir_paths:
            path.append(lsdir_path)
...
def forward(img_lq, model, tile=None, tile_overlap=50, scale=1):
    if tile is None:
...
def run(model, model_name, data_range, tile, logger, device, args, mode="test"):
    ...
    for i, (img_noisy, img_hr) in enumerate(data_path):
        ...
        num_aug = 8
        img_dn = np.zeros_like(img_noisy).astype(np.float32)

        for aug_mode in range(0, num_aug):
            img_noisy_augmented = augment_img(img_noisy, aug_mode)
            img_noisy_augmented = util.uint2tensor4(img_noisy_augmented, data_range)
            img_noisy_augmented = img_noisy_augmented.to(device)

            # --------------------------------
            # (2) img_dn
            # --------------------------------
            img_dn_augmented = forward(img_noisy_augmented, model, tile)
            img_dn_augmented = util.tensor2float(img_dn_augmented, data_range)
            img_dn += reverse_augment_img(img_dn_augmented, aug_mode)
        
        img_dn = np.divide(img_dn, 8.0)
        img_dn = np.uint8((img_dn*255.0/data_range).round())
        ...
        util.imsave_png_without_compression(img_dn, os.path.join(save_path, img_name+ext))
    ...
    print("Average psnr: ", results[f"{mode}_ave_psnr"])
    if args.ssim:
        results[f"{mode}_ave_ssim"] = sum(results[f"{mode}_ssim"]) / len(results[f"{mode}_ssim"])
        print("Average ssim: ", results[f"{mode}_ave_ssim"])
    ...
    logger.info("{:>16s} : {:<.3f} [M]".format("Max Memory", results[f"{mode}_memory"]))  # Memory
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_dir", default="../NTIRE2025_Dn50_challenge-main/input", type=str)
...
'''
For Cascaded Gaze Net inference on validation set.
1. --data-dir
2. --save-dir
3. --model_id 1
4. --ssim
